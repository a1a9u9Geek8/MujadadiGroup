...
def home(request):
    ...
    investor_relations = InvestorRelation.objects.filter(is_featured=True)[:3]
    sustainability = Sustainability.objects.filter(is_featured=True)[:3]
    news_media = NewsMedia.objects.filter(is_featured=True)[:3]
...
def investor_relations(request):
    investor_relations = InvestorRelation.objects.all()
    contact_info = ContactInfo.objects.filter(is_active=True).first()
...
def sustainability_page(request):
...
def news_media(request):
    news_media = NewsMedia.objects.all()
    contact_info = ContactInfo.objects.filter(is_active=True).first()
